 ...
 */
void BME280_Measure(volatile float *temperature, volatile float *pressure, volatile float *humidity)
{
    if (BMEReadRaw() == 0)
    {
...
static int TrimRead(void)
{
	...
	dig_H3 = trimdata[27];
	dig_H4 = (int16_t)((((int16_t)trimdata[28]) << 4) | (trimdata[29] & 0x0F));
	dig_H5 = (int16_t)((((int16_t)trimdata[30]) << 4) | (trimdata[29] >> 4));
